...
export default class ValueStream {
  ...
  private buildAndCombineStreams() {
    ...
    const retryPipeline = retryWhen((errors) =>
      errors.pipe(delay(RETRY_DELAY_MS), take(RETRY_LIMIT))
    );
  ...
  private buildObservables() {
    ...
    if (this.config.vpca_group) {
      vpca = makeWebSocketObservable(
        this.buildUrl(VPCA_APP_NAME),
        this.options
      );
    }
    let chat = null;
    if (this.config.chat_group) {
      chat = makeWebSocketObservable(
        this.buildUrl(CHAT_APP_NAME),
        this.options
      );
    }
